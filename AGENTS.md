...
- 5. Sync local branch first: `git pull --ff-only`.
- 6. Create the tag on the release commit: `git tag vX.Y.Z`.
- 7. Push the tag: `git push origin vX.Y.Z`.
- 8. Monitor the release workflow: `gh run list --workflow release.yml --limit 10`.
- 9. Verify the GitHub release page after the workflow finishes.
...
- After publishing, verify by opening the GitHub Releases page and checking the uploaded assets.
